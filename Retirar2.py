...
class Retirar2:
    ...
    def __init__(self, master):
        ...
        if len(self.auxiliar) == 1:
            label2 = tk.Label(self.frame, text="¿Retirar la batería " + escribirNombre2(self.archivos[self.auxiliar[0]]) + '?', bg='white')
...
class Retirar3:
    ...
    def __init__(self, master):
        ...
        botonConfirmar = tk.Button(self.frame, text="Eliminar batería", height=2, bg='white', command=lambda: self.confirm())
    ...
    def goHome(self):
        self.master.destroy()
        Retirar2.close_windows()
        Retirar1.close_windows()

    def confirm(self):
        nombre = getNombreArchivo()
        eliminarBateria(nombre, self.entrada.get())
        self.goHome()
